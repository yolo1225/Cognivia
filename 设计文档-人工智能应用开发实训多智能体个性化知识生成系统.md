...
> 文档版本：v1.2
> 最近同步：2026 年 8 月 27 日（以 `develop` 分支实现为准）
...
本设计文档保留早期 MVP 的背景、方案推演和数据/API 草案；其中出现的
`agent-contract-v1/v2`、`legacy_*`、`human_review` 顶层节点、演示令牌认证、未使用 Redis、
独立 Agent 协同前端页以及“手工逐步导入”的描述，均不是当前运行态。与本节冲突时，以本节、
`docs/current-iteration-plan.md`、`docs/project-conventions.md` 与可执行代码为准。

当前已实现的运行边界如下：

- 活动 Agent 契约为 `agent-contract-v9`；唯一图构建入口是
  `backend/app/agents/graphs.py` 的 `build_learning_graph()`。V9 保持六 Agent 职责和图拓扑，
  将正式分阶测试题数调整为 3–8；V8 及更早资料均为历史兼容记录。
- 每个生成事件使用自身的 `generation_tasks.public_id` 作为公开 `task_id` 与 LangGraph
  `thread_id`；反馈、学习调整和知识刷新任务通过 `source_task_id`、`source_feedback_id` 保留与来源
  学习包的关系。生成图固定为 `prepare_task -> (interpret_feedback) -> analyze_profile -> retrieve_knowledge
  -> generate_resource -> review_resource -> finalize_task`，审核失败可按 `revision_required`
  最多回流两轮；持续未解决的资源不发布，不存在学习主图中的 `human_review` 暂停节点。
- 运行时只使用已发布领域的 active Candidate manifest；索引、数据版本、embedding 模型或集合
  元数据不一致时拒绝创建生成任务。资源通过双模型事实/来源核验、字段级修订和包级质量门槛后
  原子发布。
- 当前认证使用 learner/admin 角色、bcrypt 密码、JWT HttpOnly Cookie、CSRF 校验和 Redis
  refresh session；这是一套已实现的 MVP 登录能力，而非完整企业身份体系。
- 学习者页面覆盖三步建档、资源、错题巩固、报告和学习历程；原始 Agent 运行/消息仅由受控任务
  API、SSE 和离线评测使用，学习者界面不展示完整 Agent payload。
- 领域管理已实现文档驱动批次导入：上传、候选生成与校验、图谱预览、Candidate 索引、检索冒烟、
  一次确认发布。正式题库必须经过来源 Chunk 绑定与 `pending/certified/rejected/stale` 认证门禁。
- 学习效果闭环已包括异步诊断评分、知识状态投影、前置关系路径解锁、资源内测验、错题巩固、
  导学评估和学习调整提案；主观快捷反馈本身不会直接改写画像。
...
| UI 组件 | 项目内 Vue 组件 + CSS | — | 表单、状态、弹窗、布局与指标展示；当前 `package.json` 未引入 Element Plus |
...
3. Agent 运行状态、任务、消息和检查点使用 MySQL 持久化；Redis 只用于现有 refresh session
   等运行支撑，不作为 Agent State 或通用缓存替代品。
...
8. 当前用户体系采用 learner/admin 角色、bcrypt 密码、JWT HttpOnly Cookie、CSRF 校验和 Redis
   refresh session；不建设找回密码、MFA、复杂权限审批或第三方身份集成。
...
| 可视化 | 完成工作台建档/诊断、学习资源、错题巩固、学习报告、学习历程、领域管理和模型配置页面；Agent 轨迹通过受控 API/SSE 留痕 |
...
5. Agent 输入输出 JSON 必须包含 `contract_version` 字段。正式开发契约冻结为
   `"agent-contract-v9"`，以 `docs/agent-contract-v9.md`、`backend/app/agents/contracts.py`、
   `backend/app/agents/state.py` 与 `docs/contracts/v9/` 的生成 Schema/示例为唯一事实来源；不再存在
   `legacy_*` 运行链。
...
顶层节点只保留 Agent 调用和条件路由等需要独立追踪的业务检查点。加载上下文、保存草稿、保存审核报告、更新画像、计算影响范围、发布资源和刷新路径等确定性操作由对应节点内部的应用服务完成，不再拆成独立顶层节点。证据不足时 `finalize_task` 保留原画像、路径和资源并保存追问、提示或即时解释；画像发生有效变化时，`analyze_profile` 在同一次运行中创建画像新版本并计算影响范围，再触发受影响资源的局部重生成。持续审核分歧会重新检索并进行字段级修订；两轮后仍不通过则任务失败且资源不发布，不进入人工暂停节点。
...
当前运行时使用 V9 节点专属输出组成的 `AgentGraphState`，字段以
`docs/agent-contract-v9.md`、`backend/app/agents/contracts.py` 与 `backend/app/agents/state.py`
为准。下列扁平 `AgentGraphState` 仅为历史 V1 草图，保留用于理解早期演进，不得作为新 Agent
实现、测试或数据迁移依据：
...
下表与其后的 `human_review` 路由同为历史 V1/V2 草图。当前 V9 顶层节点仅为
`prepare_task`、`interpret_feedback`、`analyze_profile`、`retrieve_knowledge`、
`generate_resource`、`review_resource`、`finalize_task` 七个节点；以
`backend/app/agents/graphs.py` 为唯一可执行定义。

历史 LangGraph 顶层节点为 8 个：
...
| `contract_version` | VARCHAR(32) | NOT NULL DEFAULT 'agent-contract-v9' | 契约版本；历史行按留存版本读取 |
| `created_at` | DATETIME | NOT NULL | 创建时间 |
...
当前认证使用 learner/admin 角色、bcrypt 密码、JWT HttpOnly Cookie、CSRF header 与 Redis
refresh session。`/auth/register`、`/auth/login`、`/auth/refresh`、`/auth/logout` 和
`/auth/me` 已实现；后续扩展不包括找回密码、MFA、复杂组织权限或第三方身份集成。
...
`POST /resources/{resource_id}/feedback` 保存反馈后，由后端自动创建 `trigger_type=resource_feedback` 的新任务，并以 `source_task_id`、`source_feedback_id` 关联来源学习包；该任务自身的 `task_id` 串联反馈识别、画像判断、资源处理、Agent 运行记录和 SSE 事件。
...
同一次生成事件的触发路由、反馈判断、画像更新、资源生成、审核和路径刷新必须使用该事件自身的 `generation_tasks.public_id` 作为 `thread_id` 和对外 `task_id`；跨事件的继承关系通过 `source_task_id` 保存。
...
4. 仲裁优先重新检索来源片段并再次审核；仍不一致时仅对受影响字段自动局部修订，最多两轮，最终未通过资源不发布给学习者。
...
| 反馈意图 | 即时动作 | 画像处理 |
|----------|----------|----------|
| `too_hard` | 先通过 AI 对话追问具体困难并返回即时解释或提示 | 单次主观反馈不更新能力画像；画像未变化时不生成新资源 |
| `too_easy` | 先通过 AI 对话确认掌握程度并返回进阶建议 | 有后续答题或行为证据时再判断；只有画像有效变化后才生成受影响的挑战资源 |
...
反馈分支必须区分三种结果：证据不足或画像未变化时只返回追问、提示或即时解释并保留原画像、路径和资源，不创建画像驱动的新资源；学习者明确报告资源事实错误时，可对原资源进入重新检索、复核和局部修订，该分支不等同于画像变化；只有画像发生有效变化时，才计算影响范围并对受影响的核心资源和路径节点重新检索、生成和审核，进阶挑战资源也遵循该条件。

交互导学策略：

1. 学习者回答错误时，先给 1 个提示问题，不直接给答案。
2. 学习者第二次仍错误时，生成降维解释。
3. 学习者在 AI 对话中连续表达内容过易，且后续答题或学习行为足以支持画像有效变化时，才生成受影响的进阶挑战资源；否则只返回进阶建议并以 `no_change` 结束。评分可作为辅助证据，不作为唯一触发条件。
...
学习者没有有效初始画像时，工作台呈现一次性“三步建档”流程，隐藏学习资源、学习报告和任务记录；诊断不是独立导航页。有效初始画像必须由“学习背景与方向采集 + 10 题诊断测评”建立，且记录诊断来源、完成状态和背景快照；反馈更新的画像版本沿用该诊断谱系与快照。
...
4. 无有效初始画像时展示三步建档；完成画像后显示画像与学习路线，并从报告页创建学习包。
...
1. 首次画像流程固定为三步以内：“填写基础信息与方向 -> 完成诊断 -> 查看画像与路线”；页面顶部使用步骤条显示进度。
...
诊断作为工作台内的第二步，不单独出现在导航栏。流程：

1. 保存学历、专业、相关经验、学习偏好与 1 至 3 个方向标签。
2. 创建固定 10 题诊断测试。
3. 答题并提交。
4. 展示得分、知识点归因和初始画像。
...
5. 页面展示本次方向标签、题型、理论/实操分类和难度分布，便于证明抽题约束生效。
...
3. 提交 `too_easy` 后先按证据规则判断画像；画像有效变化时生成受影响的挑战资源，画像未变化时保留原资源并返回进阶建议。
