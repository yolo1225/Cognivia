...
`docs/agent-contract-v8.md`、`backend/app/agents/contracts.py`、`backend/app/agents/state.py` 和对应 Schema 是当前唯一活动契约。契约由一名指定负责人统一维护，其他成员及开发代理在实现具体 Agent 时必须将以下文件视为只读：
...
当前运行链和所有独立服务入口统一使用正式 `contracts` 与 `state`。V7 及更早文档和 Schema 仅供读取历史记录；V1-V7 契约、State 和 Agent 实现已经退出活动运行链，禁止重新引入或与 V8 指标混算。
