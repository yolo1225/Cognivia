"""Shared, deterministic loading and integrity checks for Profile V3 fixtures."""
...
class ProfileFixtureError(ValueError):
    """Raised when a checked-in Profile V3 fixture is malformed or unreviewed."""


@dataclass(frozen=True)
class RenderedProfileCase:
...
def load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise ProfileFixtureError(f"invalid_profile_v3_fixture:{path.name}") from exc
    if not isinstance(value, dict):
        raise ProfileFixtureError(f"invalid_profile_v3_fixture_shape:{path.name}")
    return value
...
def rendered_case_records(
    ...
    for document in all_case_documents(fixture_dir):
        if document.get("fixture_version") != config.version:
            raise ProfileFixtureError("profile_v3_fixture_version_mismatch")
        templates = document.get("templates")
        cases = document.get("cases")
        if not isinstance(templates, dict) or not isinstance(cases, list):
            raise ProfileFixtureError("invalid_profile_v3_fixture_shape")
        for case in cases:
            if not isinstance(case, dict):
                raise ProfileFixtureError("invalid_profile_v3_case")
...
def validate_acceptance_manifest(
    ...
    if manifest.get("baseline_version") != ACCEPTANCE_BASELINE_VERSION:
        raise ProfileFixtureError("acceptance_baseline_version_mismatch")
    if manifest.get("fixture_version") != acceptance.get("fixture_version"):
        raise ProfileFixtureError("acceptance_fixture_version_mismatch")
    if manifest.get("config_version") != config.version:
        raise ProfileFixtureError("acceptance_config_version_mismatch")
    if manifest.get("seed_sha256") != config.seed_sha256:
        raise ProfileFixtureError("acceptance_seed_fingerprint_mismatch")
    if manifest.get("fixture_hash_algorithm") != CANONICAL_HASH_ALGORITHM:
        raise ProfileFixtureError("acceptance_hash_algorithm_mismatch")
    ...
    if not isinstance(expected_hashes, dict):
        raise ProfileFixtureError("acceptance_case_hashes_missing")

    cases = acceptance.get("cases")
    if not isinstance(cases, list):
        raise ProfileFixtureError("invalid_acceptance_fixture_shape")
    ...
    if set(expected_hashes) != actual_ids or len(actual_ids) != len(cases):
        raise ProfileFixtureError("acceptance_case_ids_mismatch")
    for case in cases:
        if not isinstance(case, dict):
            raise ProfileFixtureError("invalid_acceptance_case")
        case_id = case.get("case_id")
        if not isinstance(case_id, str) or expected_hashes.get(case_id) != canonical_json_sha256(case):
            raise ProfileFixtureError("acceptance_case_hash_mismatch")
    return acceptance
