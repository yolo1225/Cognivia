...
STATE_VERSION = "knowledge-state-v1"
...
_SOURCE_RELIABILITY = {
    ...
    EvidenceType.VALIDATED_BEHAVIOR: 0.8,
}
...
def _status(
    *, mastery: float, effective_weight: float, confusion_tag_counts: Mapping[str, int]
) -> str:
    if effective_weight < MIN_EFFECTIVE_WEIGHT:
        return "unassessed"
    if mastery < 0.4:
        return "unmastered"
    if mastery >= 0.8 and effective_weight >= KNOWN_EFFECTIVE_WEIGHT:
        return "known"
...
def project_analysis_with_knowledge_state(
    *,
    ...
    """Replace internal calculations while preserving the frozen V9 output shape."""
    ...
    profile_type = (
        ...
        if abilities.practice >= abilities.theory + 10 and abilities.practice >= 60
    ...
    changed_dimensions = ["knowledge_state", "ability_scores"]
    if weak != before.weak_knowledge:
        changed_dimensions.append("weak_knowledge")
    if blind_spots != before.blind_spot_ids:
        changed_dimensions.append("blind_spot_ids")
    if profile_type != before.profile_type:
    ...
    normalized = AnalyzeProfileOutput.model_validate(
        analysis.model_copy(
            update={
                ...
                "decision_reason": "已根据累计正式证据更新广义学情画像",
            }
