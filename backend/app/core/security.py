...
def create_access_token(user: User, session_id: str) -> str:
    ...
    return jwt.encode(
        {"sub": user.public_id, "role": user.role, "sid": session_id, "iat": now,
         "exp": now + timedelta(minutes=settings.access_token_minutes), "jti": str(uuid4())},
...
def get_current_user(access_token: str | None = Cookie(None), db: Session = Depends(get_db)) -> Principal:
...
def principal_learner(principal: Principal, requested: str | None = None) -> str:
    if principal.role == "admin":
        if not requested: raise HTTPException(422, "learner_id is required")
        return requested
    if not principal.learner_id: raise HTTPException(403, "当前账号未关联学习者")
    if requested and requested != principal.learner_id: raise HTTPException(403, "无权操作其他学习者")
...
def require_task(db: Session, principal: Principal, task_id: str) -> GenerationTask:
    ...
    if not task or (principal.role!="admin" and db.get(Learner,task.learner_id).public_id!=principal.learner_id): raise HTTPException(404,"Generation task not found")
    return task
...
def require_resource(db: Session, principal: Principal, resource_id: str) -> LearningResource:
    ...
    require_task(db,principal,db.get(GenerationTask,resource.generation_task_id).public_id); return resource
...
def require_tutoring(db:Session,principal:Principal,session_id:str)->TutoringSession:
    ...
    if not session or (principal.role!="admin" and learner.public_id!=principal.learner_id): raise HTTPException(404,"Tutoring session not found")
    return session
