...
class RetrievalError(RuntimeError):
    """Raised when V3 retrieval cannot safely use its candidate index."""
...
class CandidateRetriever:
    ...
    def __init__(
        ...
        *,
        ...
        if mode not in {"full", "semantic-only", "explicit-only", "semantic+relation"}:
            raise ValueError(f"unsupported V3 retrieval mode: {mode}")
        self.db = db
    ...
    def _active_collection(self, domain_code: str) -> tuple[CandidateIndexManifest, Any]:
        ...
        try:
            ...
            raise RetrievalError("candidate active collection is unavailable") from exc
        if collection.count() != manifest.indexed_chunk_count:
            raise RetrievalError("candidate collection count does not match manifest")
        ...
        for metadata_field, value in expected.items():
            if metadata.get(metadata_field) != value:
                raise RetrievalError(f"candidate collection metadata mismatch: {metadata_field}")
    ...
    def _build_query(self, request: RetrieveKnowledgeInput) -> str:
        ...
        if ResourceType.PRACTICE_GUIDE in request.context.resource_types:
            resource_terms = [
                "环境要求",
                "操作步骤",
                "命令或代码示例",
        ...
        weak_terms = [
            term for weak in request.profile.weak_knowledge for term in (weak.name, weak.category)
        ]
        revision_terms = list(request.revision_plan.query_terms) if request.revision_plan else []
        terms = _ordered_unique([*base_terms, *resource_terms, *weak_terms, *revision_terms])
        query = " ".join(terms)
        if len(query) <= 2000:
            return query
    ...
    def _embed_query(self, query_text: str, manifest: CandidateIndexManifest) -> list[float]:
        try:
            ...
            raise RetrievalError(f"candidate query embedding failed: {type(exc).__name__}") from exc
        except Exception as exc:
            raise RetrievalError(f"candidate query embedding failed: {type(exc).__name__}") from exc
        if len(vectors) != 1 or len(vectors[0]) != manifest.embedding_dimensions:
            raise RetrievalError("candidate query embedding dimensions do not match manifest")
        vector = [float(value) for value in vectors[0]]
        if not all(math.isfinite(value) for value in vector):
            raise RetrievalError("candidate query embedding contains non-finite values")
        return vector
    ...
    def _records_from_result(self, result: dict[str, Any]) -> list[CandidateRecord]:
        ...
        if not (len(ids) == len(documents) == len(metadatas) == len(embeddings)):
            raise RetrievalError("candidate collection returned inconsistent record arrays")
        return [
            CandidateRecord(
    ...
    def _explicit_records(
        ...
        try:
            ...
            raise RetrievalError("candidate explicit lookup failed") from exc
        records = self._valid_records(
            self._records_from_result(result), manifest, domain_code, warnings
        )
        records.sort(
            key=lambda record: (int(record.metadata.get("chunk_index", 0)), record.chunk_id)
        )
        if not records:
        ...
        for route in (
            ...
            for knowledge_id in _ordered_unique(route_ids[route]):
                if total >= MAX_RELATION_CANDIDATES:
                    ...
                    break
                records = self._explicit_records(
                    collection, manifest, domain_code, knowledge_id, warnings
                )
    ...
    def _semantic_records(
        ...
        if count <= 0:
            raise RetrievalError("candidate collection is empty")
        try:
            ...
            raise RetrievalError("candidate semantic query failed") from exc
    ...
    def _records_from_query_result(self, result: dict[str, Any]) -> list[CandidateRecord]:
        ...
        raw_embeddings = (
            first("embeddings") if result.get("embeddings") is not None else [None] * len(ids)
        )
        if not (
            len(ids) == len(documents) == len(metadatas) == len(distances) == len(raw_embeddings)
        ):
            raise RetrievalError("candidate semantic query returned inconsistent arrays")
        ...
        for index, chunk_id in enumerate(ids):
            ...
            records.append(
                CandidateRecord(
                    ...
                    metadata=metadata,
                    embedding=(
                        None if raw_embeddings[index] is None else list(raw_embeddings[index])
                    ),
                )
            )
        return records
    ...
    def _valid_records(
        ...
        for record in records:
            ...
            if difficulty not in {1, 2, 3, 4, 5}:
                ...
                continue
            if not _clean_text(metadata.get("source_title")) or not _clean_text(
    ...
    def _assemble_output(
        ...
        *,
        ...
        available = [
            ...
            if record.routes and record.similarity is not None
        ]
        ...
        def score(record: CandidateRecord) -> tuple[float, float, float, str]:
            ...
            if request.purpose != RetrievalPurpose.SOURCE_VERIFICATION:
                ...
                if strategy == "remedial" and difficulty > target:
                    ...
                    difficulty_score *= 0.5
            total = (
                0.50 * route_scores[route]
                + 0.35 * float(record.similarity)
        ...
        def practice_evidence_score(record: CandidateRecord) -> int:
            ...
            weights = {
                ...
                EvidenceCapability.CODE_EXAMPLE: 4,
        ...
        ranked = sorted(
            available,
            key=lambda record: (
                -practice_evidence_score(record),
                -score(record)[0],
                -score(record)[1],
                score(record)[2],
                score(record)[3],
        ...
        chunks = [
            RetrievedChunk(
                ...
                content_checksum=(
                    _clean_text(record.metadata.get("content_checksum"))
                    or hashlib.sha256(record.document.encode("utf-8")).hexdigest()
                ),
                source_locator=(
                    _clean_text(record.metadata.get("source_locator"))
                    or f"knowledge:{record.knowledge_id}#chunk={record.metadata.get('chunk_index', 0)}"
                ),
            )
