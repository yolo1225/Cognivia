...
def users(
    ...
    stmt = select(User).order_by(User.role.desc(), User.username.asc())
    stmt = stmt.where(User.status == status) if status else stmt
...
def status(
    ...
    if user.public_id == p.user_id and body.status == "disabled":
        raise HTTPException(409, "不能禁用当前账号")
    if (
        user.role == "admin"
        ...
        raise HTTPException(409, "不能禁用唯一管理员")
    user.status = body.status
    db.commit()
    if body.status == "disabled":
        session_store.revoke_user(user.public_id)
...
def revoke(
    user_id: str, db: Session = Depends(get_db), _p: Principal = Depends(require_admin)
) -> ApiResponse:
    user = find(db, user_id)
    session_store.revoke_user(user.public_id)
