"""Standalone V2 boundary for deterministic learner-profile analysis."""
...
PROFILE_ANALYSIS_AGENT_NAME = "profile_analysis_agent_v2"
SYSTEM_PROMPT = (
    "你是 V2 学情分析智能体。只基于正式结构化输入进行确定性画像分析，"
    "不解析原始反馈文本、不调用 LLM、不访问数据库或检索系统。"
)


class V2ProfileAnalysisAgent:
    """Isolated V2 Agent boundary; it deliberately does not inherit legacy BaseAgent."""
    ...
    def execute(self, request: AnalyzeProfileInput) -> AnalyzeProfileOutput:
        """Validate one V2 request, run the pure algorithm, and emit safe observability."""
