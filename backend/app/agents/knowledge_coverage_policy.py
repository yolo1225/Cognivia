...
def allocate_resource_knowledge_targets(
    ...
    # A practice guide normally requires an operational target so its steps are
    # backed by explicit 操作步骤 evidence. When every target is purely conceptual
    # (a learner weak only in theory), failing the whole generation would block
    # the lecture and quiz that ARE supportable. Fall back to a conceptual target
    # instead: the generation prompt already emits a null code_or_command when
    # command/code_example evidence is absent, so the resulting practice guide
    # stays evidence-faithful without inventing operations.
