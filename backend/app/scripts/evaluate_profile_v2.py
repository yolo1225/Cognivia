"""Run deterministic V2 profile evaluation and enforce the frozen quality gate."""
...
REPORT_SCHEMA_VERSION = "profile-v2-evaluation-v2"
ALGORITHM_VERSION = "profile_analysis_service_v2"
...
def _run_case(
    case: RenderedProfileV2Case, analyzer: Analyzer
...
def evaluate_profile_v2(analyzer: Analyzer | None = None) -> dict[str, Any]:
    validate_acceptance_manifest()
...
def main() -> None:
    try:
        report = evaluate_profile_v2()
    except (ProfileV2FixtureError, ValidationError, ProfileAnalysisError, ValueError) as exc:
        error_code = str(exc) if isinstance(exc, ProfileV2FixtureError) else "evaluation_input_failed"
        print(
            json.dumps(
                {
                    "schema_version": REPORT_SCHEMA_VERSION,
