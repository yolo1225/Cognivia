"""Replace template-permutation questions with source-grounded v3 questions.
...
V3_PATH = SEED_DIR / "question_bank_expansion.json"
...
def _insert_records(connection: sa.Connection, records: list[dict]) -> None:
    ...
    missing_knowledge_ids = required_knowledge_ids - set(knowledge)
...
def upgrade() -> None:
    ...
    # Keep historical answer records immutable, but retire the template-based
    # candidates so they can never be selected as new formal evidence.
    connection.execute(sa.text("""
        UPDATE diagnostic_questions
        SET status = 'disabled', disabled_at = :now,
            disabled_reason = 'superseded_by_source_grounded_v3_question_bank'
        WHERE public_id IN :question_ids
    """).bindparams(sa.bindparam("question_ids", expanding=True)), {
        "question_ids": v2_ids,
        "now": datetime.now(UTC).replace(tzinfo=None),
    })
    _insert_records(connection, _records(V3_PATH))


def downgrade() -> None:
    connection = op.get_bind()
    v3_ids = [str(record["question_id"]) for record in _records(V3_PATH)]
    if _has_v1_references(connection, v3_ids):
        raise RuntimeError("question_bank_v3_in_use")
    for question_id in v3_ids:
    ...
    connection.execute(sa.text("""
        ...
        WHERE public_id IN :question_ids
    """).bindparams(sa.bindparam("question_ids", expanding=True)), {"question_ids": v2_ids})
