...
def test_profile_output_is_directly_consumable_by_v2_retrieval_contract() -> None:
    ...
    for case_id, payload, _ in rendered_cases():
        ...
        assert retrieval_input.context.contract_version == "agent-contract-v2", case_id
...
def test_review_uses_source_verification_purpose() -> None:
    ...
    output = V2ProfileAnalysisAgent().execute(review_request)
    retrieval_input = _to_retrieval_input(
        review_request,
        profile=output.profile,
        retrieval_plan=output.retrieval_plan,
