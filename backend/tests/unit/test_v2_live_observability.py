...
from app.agents.v2_review_agent import OpenAICompatibleReviewChannel, V2ReviewValidationAgent
...
def test_v2_generation_and_review_collect_safe_model_call_metadata() -> None:
    flow = initial_generation_flow_example()
    generation = V2ContentGenerationAgent(
    ...
    review = V2ReviewValidationAgent(channel=OpenAICompatibleReviewChannel(_Gateway()))
    with collect_model_calls() as collector:
...
def test_live_runner_consumes_v2_review_summary_and_rejects_non_live_calls() -> None:
...
def test_live_runner_resolves_v2_source_reference_to_knowledge_id() -> None:
    root = Path(__file__).resolve().parents[3]
    sys.path.insert(0, str(root / "test_script"))
    import run_live
