...
from app.scripts.validate_v2_rag_admission import (
    AdmissionPaths,
    markdown_report,
    validate_v2_rag_admission,
)
...
def _manifest_and_identity() -> tuple[CandidateIndexManifest, dict[str, str]]:
    ...
    return manifest, {
        ...
        "algorithm_version": "v2-candidate-test-1.0",
    ...
    return {
        ...
        **identity,
        "target_checks": {
            ...
            "p95_latency_ms": True,
            "v2_contract_illegal_outputs": True,
        },
        "metrics": {
...
def test_inconsistent_index_or_acceptance_hash_is_rejected(tmp_path: Path) -> None:
    ...
    index_result = validate_v2_rag_admission(
        paths, collection_metadata_reader=_collection_reader(manifest)
    )
    ...
    _write_json(paths.acceptance, acceptance)
    hash_result = validate_v2_rag_admission(
        paths, collection_metadata_reader=_collection_reader(manifest)
    )
