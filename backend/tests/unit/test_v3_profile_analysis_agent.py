...
from app.agents.profile_analysis_agent import (
...
def test_v3_agent_preserves_controlled_evidence_conflict_error() -> None:
...
def test_v3_agent_logs_only_safe_summary_fields(caplog: pytest.LogCaptureFixture) -> None:
    payload = _input_for_case("accept-initial-01").model_dump(mode="python")
...
def test_v3_agent_has_no_legacy_or_runtime_dependencies() -> None:
    source = Path(__file__).parents[2] / "app" / "agents" / "profile_analysis_agent.py"
