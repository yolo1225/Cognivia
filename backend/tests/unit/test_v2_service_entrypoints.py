...
def test_diagnostic_entrypoint_persists_v2_profile_path_and_safe_observability() -> None:
    ...
    with testing_session() as db:
        learner, choice, short = seed_diagnostic_fixture(db)
        result = _submit(db, learner, choice, short, session_id="diag_v2")
        run = db.query(AgentRun).filter_by(agent_name="profile_analysis_agent").one()
        messages = db.query(AgentMessageRecord).filter_by(session_id="diag_v2").all()
...
def test_repeated_diagnostic_creates_a_new_v2_profile_version_and_refreshes_old_path() -> None:
    testing_session = build_test_session()
    with testing_session() as db:
        learner, choice, short = seed_diagnostic_fixture(db)
        first = _submit(
