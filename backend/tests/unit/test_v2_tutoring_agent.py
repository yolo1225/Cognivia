...
from app.agents.v2_tutoring_agent import (
    SYSTEM_PROMPT,
    TUTORING_AGENT_NAME,
    TutoringAgentError,
    V2TutoringAgent,
...
def _execute(request: InterpretFeedbackInput, semantic: Any) -> InterpretFeedbackOutput:
    return V2TutoringAgent(interpreter=FakeInterpreter(semantic)).execute(request)


def test_v2_agent_exposes_identity_and_model_boundary_prompt() -> None:
    agent = V2TutoringAgent(interpreter=FakeInterpreter(_semantic(FeedbackIntent.TOO_HARD)))
...
def test_agent_rejects_raw_dict() -> None:
    with pytest.raises(TutoringAgentError, match="invalid_interpret_feedback_input_type"):
        V2TutoringAgent(interpreter=FakeInterpreter(_semantic(FeedbackIntent.TOO_HARD))).execute(  # type: ignore[arg-type]
...
def test_logs_safe_identifiers_without_feedback_body(caplog: pytest.LogCaptureFixture) -> None:
    ...
    request = InterpretFeedbackInput.model_validate(payload)
    logger = logging.getLogger("tests.v2_tutoring_agent")
...
def test_agent_has_no_legacy_graph_or_database_dependency() -> None:
    source = Path(__file__).parents[2] / "app" / "agents" / "v2_tutoring_agent.py"
