...
import app.scripts.evaluate_profile_v2 as evaluation
...
def _report_with_mutation(mutator):
    ...
    return evaluation.evaluate_profile_v2(analyzer=analyzer), target_case_id
...
def test_main_exits_nonzero_for_a_failed_quality_report(monkeypatch, capsys) -> None:
    monkeypatch.setattr(evaluation, "evaluate_profile_v2", lambda: {"status": "failed"})
...
def test_current_frozen_baseline_meets_all_quality_thresholds() -> None:
    report = evaluation.evaluate_profile_v2()
