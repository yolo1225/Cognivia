...
def test_v2_graph_executes_initial_generation_with_contract_owned_state() -> None:
...
def test_v2_graph_stops_after_two_revisions_using_prior_finalize_output() -> None:
    flow = initial_generation_flow_example()
    prepare = flow["prepare_task"]
    analyze = flow["analyze_profile"]
    retrieve = flow["retrieve_knowledge"]
    ...
    revision_output = review["output"].model_copy(update={"reports": [revision_report]})
