...
@pytest.mark.live
def test_live_v3_retrieval_uses_active_candidate_manifest() -> None:
    if os.getenv("RUN_LIVE_EMBEDDING_TESTS", "").lower() != "true":
        pytest.skip("set RUN_LIVE_EMBEDDING_TESTS=true to enable paid V3 retrieval")
    settings = Settings()
    ...
    with KnowledgeRetrievalAgent.production() as agent:
